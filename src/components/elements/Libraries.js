			...
			if (library) {
				return (
		        	<div key={libraryName} className="libraries__item">
		        		<span className="libraries__item__link libraries__item__link--enabled">{libraryName}</span>
...
const Tooltip = ({ library, variant }) => {
	...
	return (
		<div className={classNames('library__tooltip', {'arrow-top': variant === 'horizontal'}, {'arrow-right': variant === 'vertical'})}>
			<div className="toolip__topzone"/>
			<div className="tooltip__inner">
				<h3 className="tooltip__title">
					<a className="tooltip__title__homepage" href={library.homepage}>{library.name}</a>
					<a  className="tooltip__title__stars" href={githubUrl}><Star/>{library.stars.toLocaleString()}</a>
				</h3>
				...
				<ul>
					...
					<li><a href={bestofjsUrl}>BestOfJS</a></li>
				</ul>
			</div>
		</div>
	)
