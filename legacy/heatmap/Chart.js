...
const Row = props => {
    ...
    return (
        <tr className={classNames({ 'row-highlighted': props.highlight === rows[index] })}>
            ...
            {currentRow.map((v, i) => <Cell {...props} key={i} index={i} value={v} />)}
        </tr>
    )
}
...
class Cell extends React.Component {
    ...
    render() {
        ...
        if (
            ...
            this.props.disabledValues.indexOf(parseInt(value, 10)) !== -1
        ) {
            color = '#dadada'
        } else if (value < 0) {
            color = tinycolor('#4500EE')
                .setAlpha(alpha)
                .toRgbString()
        } else {
            color = tinycolor('#E60932')
                .setAlpha(alpha)
                .toRgbString()
        }
        return (
                ...
                <table className="heatmap-table rtable--flip">
                    <thead>
                        <tr>
                            ...
                            {columns.map(c => (
                                <th key={c} className="column-heading">
                                    <span>{c}</span>
                                </th>
                            ))}
                        </tr>
