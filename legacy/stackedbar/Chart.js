...
export default class Chart extends React.Component {
    ...
    getFill(responseName) {
        ...
        return includes(response.filters, this.props.filter)
            ? response.onColor
            : `url(#p${response.offColor.replace('#', '')})`
    }
    ...
    renderTitle() {
        return (
            <text fill="black" x="50%" y="50" textAnchor="middle" style={{ fontSize: 24 }}>
                {this.props.title}
            </text>
        )
    }
    ...
    renderChart() {
        ...
        const margins = isExport
            ? { top: 70, right: 20, left: 20, bottom: 70 }
    ...
    render() {
        return this.props.isExport ? (
            ...
            <div className="chart-wrapper">
                <ResponsiveContainer minWidth={500} aspect={1.5}>
                    {this.renderChart()}
                </ResponsiveContainer>
            </div>
        )
