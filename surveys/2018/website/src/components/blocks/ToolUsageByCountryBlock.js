...
export default class ToolUsageByCountryBlock extends Component {
    render() {
        const { tool, data, projects } = this.props

        return (
            <div className="ToolUsageByCountry__Block block">
                <BlockTitle chart="tool-usage-by-country" tool={tool} projects={projects} />
                <div className="block block--text">
                    On average, <strong>{data.percentage}%</strong> of respondents have used{' '}
                    <strong>{getToolName(tool, projects)}</strong> and would be happy to use it again.
                ...
                <ChartContainer height={'auto'}>
                    <ToolUsageByCountryMapChart data={data} tool={tool} average={data.percentage} projects={projects} />
                </ChartContainer>
            </div>
        )
    }
